...
作業の合間に別の作業(ドキュメント整備、調査、雑談的なやり取り)を挟むと、
PR作成後にマージせず放置してしまうことがあった(実例: #29対応のPRを作った後、
別の作業ブランチの上でWORKFLOW_NOTES.mdのコミットを進めてしまい、PRがマージ
されないまま残っていた)。

**対策**:
- **会話の区切り(トピックが変わるタイミング、作業を締めくくるタイミング)では
  必ず `gh pr list --repo Akatuki1121/Purge --state open` でオープンPRの有無を確認する。**
  1件でも残っていればCI状況を確認しマージする。
- ドキュメント整備のような「Issue対応ではない作業」をする前にも、まず
  `git status` でどのブランチにいるかを確認し、mainではなく作業ブランチの上に
  いる場合は要注意(そのブランチのPRがまだ残っていないか確認する)。
