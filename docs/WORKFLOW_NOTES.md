...
- **致命的バグ(機能が完全に動かない系)は即リリース**。例: 決済URLの誤記(#21)、
  ...
  他の作業完了を待たない。
- **ユーザー影響の大きいUI改善**がまとまった内容として完結していれば、そのタイミングで
  リリースに含めてよい。ただし「まとまった」かどうかも内容で判断し、機械的な件数では
  区切らない。
- **実機で未検証の変更**(特にインストーラー周り)が残っている間はリリースしない。
  一度も実機確認していない状態で配布するとリスクが高い。
- 迷ったら「今リリースしないことで、既存ユーザーが実害を受け続けるか」を基準に考える。
  受けるなら即リリース、受けないなら他の変更と合わせて出すか判断する。
